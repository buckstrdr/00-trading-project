...
import { TickAggregator } from '../services/tick-aggregator/TickAggregator.js';
...
// Configuration
const SUPPORTED_TIMEFRAMES = ['1m', '5m', '15m', '1h'];
...
app.get('/api/historical/:symbol/:timeframe', async (req, res) => {
    try {
        ...
        const historicalData = generateSessionHistoricalData(symbol, timeframe, parseInt(bars));
        res.json({
            symbol,
            timeframe,
            bars: historicalData,
...
function generateSessionHistoricalData(symbol, timeframe, bars) {
    const now = Date.now();
    ...
    for (let i = bars - 1; i >= 0; i--) {
        ...
        const open = i === bars - 1 ? currentPrice : historicalBars[historicalBars.length - 1]?.close || currentPrice;
...
function setupTickAggregator() {
    ...
    const subscribeToSymbol = (symbol) => {
        ...
        for (const timeframe of SUPPORTED_TIMEFRAMES) {
            tickAggregator.subscribe(symbol, timeframe, (candle) => {
